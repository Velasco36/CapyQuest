...
export default function TreasureZoneImproved() {
  ...
  // Inicializar ubicación y vista del mapa
  useEffect(() => {
  ...
  // Actualizar vista cuando cambia la ubicación
  useEffect(() => {
    if (userLocation) {
      setViewState(prev => ({
        ...prev,
  ...
  // Distribución de NFTs - MANTENEMOS TODAS LAS FUNCIONES
  ...
  // Navegar a la página de detalles del NFT
  const handleNFTMarkerClick = (nft: any) => {
    router.push(`/detail-nft?tokenId=${nft.tokenId}&rarity=${nft.rarity}`)
  }
  ...
  // Obtener imagen según la rareza del NFT
    ...
    return savedPolygons.map(polygon => ({
      ...
      geometry: {
        ...
        coordinates: [[
          ...
          [polygon.coordinates[0].lng, polygon.coordinates[0].lat]
        ]]
      },
      properties: {
        name: polygon.name,
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50">
      ...
      <div className="bg-white/95 backdrop-blur-sm border-b border-amber-200 shadow-sm p-3">
        <h1 className="text-xl font-bold bg-gradient-to-r from-yellow-600 to-orange-500 bg-clip-text text-transparent text-center flex items-center justify-center gap-2">
          ...
          Treasure Zone - Mejorado
        </h1>
      </div>

      {/* Tab Navigation - MANTENEMOS LAS 3 PESTAÑAS */}
      ...
      {/* Distribution Confirmation Modal - MANTENEMOS */}
